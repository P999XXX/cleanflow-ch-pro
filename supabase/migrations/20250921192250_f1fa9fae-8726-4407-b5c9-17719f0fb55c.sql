...
USING (
  ...
  auth.uid() = user_id 
  OR 
  -- Benutzer können Rollen in Firmen sehen, zu denen sie Zugang zur Firma haben
  company_id IN (
    SELECT companies.id
    FROM companies
...
WITH CHECK (
  ...
  role IN ('masteradministrator', 'administrator', 'objektleiter', 'reinigungsmitarbeiter')
);
